Feature: Audit rules-only mode (deterministic)
  ...
  Scenario: LLM findings parser handles a valid JSON object
    When I parse the LLM response '{"findings":[{"commit_sha":"abc","severity":"significant","detail":"violates DDD"}]}'
  ...
  Scenario: LLM findings parser falls back to minor on invalid severity
    When I parse the LLM response '{"findings":[{"commit_sha":"abc","severity":"critical","detail":"x"}]}'
