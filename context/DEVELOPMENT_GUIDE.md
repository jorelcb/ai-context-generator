...
- **Locale-aware** (LLM-generated artifacts): `templates/{locale}/{preset}/` for context, `templates/{locale}/workflows/` for workflows, `templates/{locale}/sdd/` for specs, `templates/{locale}/languages/` for idioms.
- **Locale-free** (English-only distributable artifacts): `templates/skills/{dir}/` for skills and `templates/hooks/{preset}/` for hooks. For skills, `--locale` still governs the LLM output language in personalized mode; the template body is English.
