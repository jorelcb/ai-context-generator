...
| `codify catalog`           | Browse and install ecosystem packages (skills, hooks). Interactive wizard, `--list` to browse, or `--type/--package/--scope` for non-interactive install. MVP: Claude. | Workstation + Project | No                               | ADR-0010                                               |
...
> **Track D (ADR-0010)**: `codify catalog` is the unified surface that will absorb the `skills`/`hooks` interactive UX. The MVP ships catalog alongside them (Claude skills + hooks, embedded source); the standalone commands are folded in incrementally. `workflows` stays standalone (it is lifecycle-coupled, not a navigable catalog item).
