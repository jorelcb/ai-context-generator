...
| `codify catalog`           | Browse and install ecosystem packages — **skills, hooks, and plugins**. Interactive wizard, `--list` to browse, or `--type/--package/--scope` for non-interactive install. `--mode personalized` LLM-adapts skills; `--type plugin` reads a Claude marketplace (`--marketplace`, default `anthropics/claude-plugins-official`) and delegates install to the `claude` CLI. MVP: Claude. | Workstation + Project | No (static) / Yes (personalized) | ADR-0010, ADR-0012                                     |
...
> **Track D (ADR-0010)**: `codify catalog` is the unified surface for skills + hooks. The standalone `codify skills` and `codify hooks` commands were **removed** in favor of it (static install for skills + hooks; `--mode personalized` for LLM-adapted skills). `workflows` stays standalone (it is lifecycle-coupled, not a navigable catalog item). Catalog is Claude-only for now; other ecosystems return in later sub-milestones.
