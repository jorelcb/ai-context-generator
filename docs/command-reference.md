...
| `codify catalog`           | Browse and install ecosystem packages — **skills and hooks**. Interactive wizard, `--list` to browse, or `--type/--package/--scope` for non-interactive install. `--mode personalized` LLM-adapts skills (`--context`/`--model`). MVP: Claude. | Workstation + Project | No (static) / Yes (personalized) | ADR-0010                                               |
