...
## 🧰 Equip (install context, catalog packages, workflows, specs)
...
| `codify catalog`           | Browse and install ecosystem packages — **skills, hooks, and plugins**. `--ecosystem claude\|antigravity` (Antigravity = skills only). Interactive wizard, `--list` to browse, or `--type/--package/--scope` for non-interactive install. `--mode personalized` LLM-adapts skills; `--type plugin` reads a Claude marketplace (`--marketplace`, default `anthropics/claude-plugins-official`) and delegates install to the `claude` CLI. `--uninstall` removes packages and drops them from the lockfile. | Workstation + Project | No (static) / Yes (personalized) | ADR-0010, ADR-0012                                     |
| `codify workflows`         | Install workflow files (multi-step recipes: bug-fix, release-cycle, spec-driven-change). `--install global\|project`.                                                                                                                                                                                                                                                                                                                                                                                     | Workstation + Project | No (static) / Yes (personalized) | [README §Workflows](../README.md#-workflows)           |

> **Track D (ADR-0010)**: `codify catalog` is the unified surface for skills + hooks (+ plugins). The standalone `codify skills` and `codify hooks` commands were **removed** in favor of it. `workflows` stays standalone (lifecycle-coupled, not a navigable catalog item). Multi-ecosystem: Claude (skills/hooks/plugins) + Antigravity (skills); Gemini CLI is deprecated and unsupported (ADR-0012 §4).
...
| `codify catalog --status`        | Compare codify's lockfile (`~/.codify/workstation.lock`, `.codify/project.lock` — what it recorded installing) against what's actually on disk. Flags packages that drifted (recorded but now missing). Both scopes by default, or one with `--scope`. No LLM. | Workstation + Project | No                              | ADR-0010 §6                                                                          |
| `codify catalog --sync`          | Re-apply a scope's lockfile: reinstall every recorded package missing on disk (reproduce an environment from a committed lockfile, or repair drift). Skills/hooks restore from the static/local catalog, plugins from their recorded marketplace ref. No LLM.  | Workstation + Project | No                              | ADR-0010 §6                                                                          |
