...
| `codify catalog --sync`          | Re-apply a scope's lockfile: reinstall every recorded package missing on disk (reproduce an environment from a committed lockfile, or repair drift). Static catalog only — plugins skipped (marketplace ref not yet stored). No LLM.                           | Workstation + Project | No                              | ADR-0010 §6                                                                          |
