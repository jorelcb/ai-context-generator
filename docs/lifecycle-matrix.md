...
| `codify catalog`   | Workstation + Project | Browse & install ecosystem packages — skills, hooks, plugins. Interactive, or `--type/--package/--scope`. `--ecosystem claude\|antigravity`. `--status`/`--sync`/`--uninstall` manage the install lockfile. |
