...
- **`codify catalog` command** — the unified surface for browsing and installing ecosystem packages (ADR-0010): **skills, hooks, and plugins**. Interactive wizard (ecosystem → type → mode → packages → scope), `--list` browsing with source badges and installed markers, and non-interactive install (`--type skill|hook|plugin --package a,b --scope project|workstation`). Skills support `--mode personalized` (LLM-adapts the template to your project via `--context`/`--model`), modeled as a `PersonalizingSource` decorator so the install path is unchanged. **Plugins** (`--type plugin`) are read from a Claude plugin marketplace (`--marketplace`, default `anthropics/claude-plugins-official`) and installed by delegating to the native `claude` plugin CLI. It is now the single surface for skills + hooks (the standalone `codify skills`/`codify hooks` commands were removed). MVP covers the Claude ecosystem.
- **Lockfile (`~/.codify/workstation.lock`, `.codify/project.lock`)** — `codify catalog` now records every install into a per-scope JSON lockfile (id, target, version, source kind, **source URI**, checksum, installedAt), sorted and atomically written (ADR-0010 §6, D.9). The source URI (the marketplace ref for plugins, the directory for local-fs) lets `--sync` rebuild a package's originating source on a fresh machine. It is codify's reproducible record of what it installed — the source of truth for drift detection, uninstall, and re-applying an environment. Wired via an optional `InstallRecorder` port on `CatalogService` (`infrastructure/lockfile`), so the orchestration stays decoupled; covers all ecosystems/types.
- **`codify catalog --status`** — the first lockfile reader (D.9·2): compares codify's recorded installs (the lockfile) against what's actually on disk and reports drift per package — `✓` in-sync vs `✗` missing (recorded but removed by hand or never finished installing). Reports both scopes by default, or one with `--scope`. Built on an optional `LockfileReader` port + `CatalogService.Status` returning a `DriftReport`, so the drift logic is unit-tested independent of presentation.
- **`codify catalog --uninstall`** — removes packages and drops them from the lockfile (D.9·2): `--uninstall --type skill|hook|plugin --package a,b --scope project|workstation` deletes the files (delegating to the same per-ecosystem installer that installed them) and then forgets the entries so the lockfile keeps reflecting reality. Uninstall is idempotent — running it on an already-gone package still prunes its drifted lockfile entry. Built on `CatalogService.Uninstall` + an optional `LockfileForgetter` port; plugins thread `--marketplace` through to the `claude` plugin CLI.
- **`codify catalog --sync`** — re-applies a scope's lockfile (D.9·2): reinstalls every recorded package that is missing on disk, so a committed `.codify/project.lock` (or `~/.codify/workstation.lock`) reproduces an environment on a fresh machine/repo — and repairs the drift `--status` flags. Reuses `Status` (to find what's missing) and `Install` (per target), so the restore runs the same paths as a fresh install and re-records idempotently. Restores skills/hooks/Antigravity skills from the static/local catalog and **plugins from their recorded marketplace ref** (`Source.URI`, sub-grouped by ref); only legacy entries that predate the recorded URI are skipped.
- **`packagesource.LocalDirectorySource` + `CompositeSource`** — personal/team packages: drop skills/hooks under `~/.codify/sources/<id>/` (a `codify-package.yaml` manifest + `SKILL.md`, or `hooks.json` + scripts) and they appear in `codify catalog` badged `[local-fs]`, composed with the built-in catalog. Local packages override built-ins of the same ID. The catalog now reads from a `CompositeSource(embedded, local)` (and `CompositeSource(personalizing(embedded), local)` in personalized mode); Fetch routes back to each manifest's originating source by `Source.Kind`. (Track D D.2.a — see ADR-0012 for the source/marketplace-standards landscape.)
- **`packagesource.PluginMarketplaceSource`** (read side) — reads a Claude-style `.claude-plugin/marketplace.json` catalog (GitHub `owner/repo` shorthand or a direct URL) and emits one `TargetClaudePlugin` manifest per plugin entry, badged `[claude-marketplace]`. The git/URL fetch is injectable; plugin `Fetch` is intentionally empty (materialization is delegated to the agent's native plugin CLI — see ADR-0012 §3, validated by the 2026-05-30 spike). (Track D D.2.b·1.)
- **`targetinstaller.ClaudePluginInstaller`** (write side) — installs Claude plugins by delegating to the native `claude` plugin CLI (`marketplace add` + `plugin install <id>@<marketplace> -s user|project`), the spike-validated "B-CLI" mechanic. `Uninstall` and `InstalledList` (reads `~/.claude/plugins/installed_plugins.json`) likewise delegate/read rather than re-implement the agent's cache. The exec is behind an injectable `CommandRunner` (unit-tested with a fake); a clear actionable error is returned when `claude` is not on `PATH`. Marketplace refs are normalized to `https://` to avoid SSH-clone dependency. `CLAUDE_CONFIG_DIR` is honored so codify and the agent agree on the config they read/write. Wired into `codify catalog --type plugin` (D.2.b·3): `--list`, non-interactive install, and a "Plugins" interactive tab; default marketplace `anthropics/claude-plugins-official`, overridable with `--marketplace`. (Track D D.2.b.)
- **Antigravity CLI ecosystem (`codify catalog --ecosystem antigravity`)** — second ecosystem, validating the cross-ecosystem port design (ADR-0012 §5). `AntigravitySkillSource` re-frames the built-in skills with Antigravity (Agent Skills) frontmatter; `AntigravityInstaller` writes them flat as `<id>.md` under `~/.gemini/antigravity-cli/skills/` (workstation) or `<project>/.agents/skills/` (project) — a filesystem write, no CLI delegation. The catalog gains `--ecosystem claude|antigravity` routing (source + installer) and an ecosystem step in the wizard. **Restores the multi-ecosystem skill install the D.4.b fold deferred.** Antigravity is skills-only for now: its plugin marketplace model is immature (a spike found `agy` cannot add arbitrary marketplaces), so Antigravity plugins are out of scope.
...
- **Interactive resolution of `[DEFINE]` markers at the end of `codify generate` / `codify analyze` / `codify init`.** Previously the user got a list of _"L42 [DEFINE: ISO 4217 currency code]"_ and had to alt-tab to their editor to fill each one in by hand. Now after the file listing prints, if any markers were emitted across the generated files, Codify offers:
...
- **`codify init` now offers project-scoped skills/workflows/hooks installs at the end of bootstrap.** Mirroring the global flow that `codify config` runs in v2.0.1, after `init` finishes generate/analyze it walks through the same opt-in steps but with `--install project` semantics:
  ...
  Previously `init` just printed _"Recommended next steps: codify skills…"_ and exited — the user had to run three separate commands. Now they all run inline in the same wizard. Each step is opt-in with `skip` as default, so a user who only wants the context can hit Enter through and end up with the same artifacts as before.
...
- **Preset labels in skill prompts now show file count.** `codify config` and `codify init` previously showed labels like _"Clean + DDD (DDD, BDD, CQRS, Hexagonal port skill)"_ — clear about _what's_ in the bundle but not how _many_ skills the bundle contains, so users picking one preset and getting 4-5 SKILL.md files would think _"all of them got installed"_. Labels now end with `— N skill(s)` so the size is upfront. Same treatment applied to workflow preset labels.
...
- **SOFT first-time prompt now fires on more commands.** `isInteractiveSuitable` was previously gated to `generate, analyze, spec, skills, workflows, hooks, init`. Natural first-contact commands like `codify usage`, `codify check`, etc. would silently skip the _"Codify isn't configured globally yet — run interactive setup now?"_ prompt. Whitelist widened to include `usage, check, audit, update, watch, list, reset-state`. `--help`, `--version`, `serve`, and `config` itself remain excluded by design.

### Changed

- **Wizard reordered + global skills/hooks install opt-in.** The `codify config` wizard previously asked `preset → locale → target → model`, with architecture preset as the _first_ question. Architecture preset is project-scoped by nature — what global config really benefits from is letting the user opt into agent-wide skills/hooks. New flow:
...
The README was rewritten to reflect the lifecycle custodian framing without changing the underlying capabilities. New tagline: _"Generate, audit, and evolve your AI agent's context across the whole project lifecycle."_ Problem section now articulates the drift problem explicitly. Before/After example extended to "Day 22" showing the watcher/check/update flow.
...
- **Severity classification is fixed**, not configurable in v1.23: `artifact_modified`, `artifact_missing`, `signal_changed`, `signal_removed` are _significant_; `artifact_new`, `signal_added` are _minor_. `--strict` flips minor into failure mode for users who want zero tolerance.
